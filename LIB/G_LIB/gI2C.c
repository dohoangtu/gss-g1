...
static uint32_t I2C_Addr(uint8_t DevAddr, uint8_t dir, I2C_TypeDef* I2Cx);
static uint32_t I2C_Write(uint8_t byte, I2C_TypeDef* I2Cx);
static uint32_t I2C_Read(uint8_t *pBuf, I2C_TypeDef* I2Cx);


static uint32_t WaitSR1FlagsSet (uint32_t Flags, I2C_TypeDef* I2Cx);
...
void I2CG_Init(void)
{
  ...
  //Configure and initialize the GPIOs
	gpioSetMode(GPIO_Pin_6 | GPIO_Pin_7, GPIO_Mode_AF, GPIO_OType_OD, GPIO_Speed_100MHz, GPIO_PuPd_UP, GPIOB);
  ...
  I2C_InitStructure.I2C_OwnAddress1 = 1; //We are the sleva. We don't need this
...
void I2C_LowLevel_Init(I2C_TypeDef* I2Cx) {
  ...
  //Configure and initialize the GPIOs
	gpioSetMode(GPIO_Pin_SCL, GPIO_Mode_AF, GPIO_OType_OD, GPIO_Speed_100MHz, GPIO_PuPd_UP, GPIO_SCL);
	gpioSetMode(GPIO_Pin_SDA, GPIO_Mode_AF, GPIO_OType_OD, GPIO_Speed_100MHz, GPIO_PuPd_UP, GPIO_SDA);
...
uint32_t I2C_WrData(uint8_t DevAddr, uint8_t RegAddr, uint8_t data, I2C_TypeDef* I2Cx){
  ...
  I2C_Addr(DevAddr, I2C_Direction_Transmitter, I2Cx);
  (void) I2Cx->SR2;
  
  //Send Data
  I2Cx->DR = data;
  WaitSR1FlagsSet(I2C_SR1_BTF,I2Cx);  //wait till the data is actually written.
...
uint32_t I2C_RdData(uint8_t DevAddr, uint8_t RegAddr, uint8_t *buf, uint32_t cnt, I2C_TypeDef* I2Cx) {
  ...
  (void) I2Cx->SR2;
  I2Cx->DR = RegAddr;
  WaitSR1FlagsSet(I2C_SR1_BTF,I2Cx);
  
  //Start Reading
  I2C_RdBuf(DevAddr, buf, cnt, I2Cx);
...
uint32_t I2C_RdBufEasy (uint8_t DevAddr, uint8_t *buf, uint32_t cnt, I2C_TypeDef* I2Cx) {
  ...
  //Send I2C Device Address and clear ADDR
  I2C_Addr(DevAddr, I2C_Direction_Receiver,I2Cx);
  (void)I2Cx->SR2;
...
uint32_t I2C_RdBuf (uint8_t DevAddr, uint8_t *buf, uint32_t cnt, I2C_TypeDef* I2Cx) {
  ...
  if (cnt==1) {//We are going to read only 1 byte
    ...
    //Wait for the BTF
    WaitSR1FlagsSet(I2C_SR1_BTF,I2Cx); //N-1 is in DR, N is in shift register
...
static uint32_t I2C_Read(uint8_t *pBuf, I2C_TypeDef* I2Cx) {
    ...
    err=WaitSR1FlagsSet(I2C_SR1_RXNE,I2Cx);
...
static uint32_t I2C_Write(uint8_t byte, I2C_TypeDef* I2Cx){
  ...
  return WaitSR1FlagsSet(I2C_SR1_TXE,I2Cx);
...
static uint32_t I2C_Addr(uint8_t DevAddr, uint8_t dir, I2C_TypeDef* I2Cx) {
  ...
  return WaitSR1FlagsSet(I2C_SR1_ADDR,I2Cx); 
...
static uint32_t I2C_Start(I2C_TypeDef* I2Cx) {
  ...
  return WaitSR1FlagsSet(I2C_SR1_SB,I2Cx);  //Wait till SB is set
